#!/usr/bin/env python3
"""
Tests for adopt_prometheus_crds function in last9-otel-setup.sh

Mirrors tests/test_adopt_otel_crds.py. Uses a mock kubectl binary
(PATH override) — no cluster needed. The mock responds to
`get crd -o name` with monitoring.coreos.com CRD names and the function
must adopt them into the last9-k8s-monitoring Helm release.

Run: python3 tests/test_adopt_prometheus_crds.py
"""
...
MOCK_KUBECTL = textwrap.dedent("""\
    ...
    case "$*" in
        "get crd -o name")
            ...
            printf 'crd/alertmanagerconfigs.monitoring.coreos.com\\ncrd/prometheuses.monitoring.coreos.com\\n'
            ;;
        *"metadata.labels.app"*)
            [ "$SCENARIO" = "already_owned" ] && echo "Helm" || echo ""
            ;;
        *"metadata.annotations.meta"*)
            [ "$SCENARIO" = "already_owned" ] && echo "last9-k8s-monitoring" || echo ""
            ;;
        *"-o yaml"*)
...
def run_adopt(scenario: str) -> list[str]:
    """Run adopt_prometheus_crds with mocked kubectl, return list of kubectl calls."""
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"adopt_prometheus_crds failed (exit {result.returncode})\n"
            f"stderr: {result.stderr[:500]}"
        )
    return calls


class TestAdoptPrometheusCRDs(unittest.TestCase):
    ...
    def test_unowned_crds_correct_release_name(self):
        """Annotate sets meta.helm.sh/release-name=last9-k8s-monitoring."""
        calls = run_adopt("unowned_crds")
        annotate_calls = [c for c in calls if c.startswith("annotate --overwrite")]
        self.assertTrue(
            all("meta.helm.sh/release-name=last9-k8s-monitoring" in c for c in annotate_calls),
    ...
    def test_no_crds_exits_cleanly(self):
        """No Prometheus CRDs in cluster — function exits 0 without any patch calls."""
